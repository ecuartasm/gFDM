...
public:
	...
	explicit NRvector(long n);		// Zero-based array
	NRvector(long n, const T &a);	//initialize to constant value
	NRvector(long n, const T *a);	// Initialize to array

	NRvector(const NRvector &rhs);	// Copy constructor
	NRvector & operator=(const NRvector &rhs);	//assignment
	NRvector & operator=(const T val);	//constant assignment
	typedef T value_type; // make T available externally
	inline T & operator[](const int i);	//i'th element
	inline const T & operator[](const int i) const;
	inline int size() const;
	inline void resize(int newn); // resize (contents not preserved)
	inline void resize(long newn); // resize (contents not preserved)
	inline void assign(int newn, const T &a); // resize and assign a constant value
	inline void zeros();

	// output operator
	friend std::ostream& operator<<(std::ostream& os, const NRvector<T>& x) {
		os << " [ ";
		for (int i=0; i < x.size(); i++)   {
			os << setprecision(8) << setw(6) << x[i] << " ";
		}
		os << " ]" << endl;
		return os;
	}
...
template <class T>
NRvector<T>::NRvector(long n) : nn(n), v(n>0 ? new T[n] : NULL) {}

template <class T>
NRvector<T>::NRvector(long n, const T& a) : nn(n), v(n>0 ? new T[n] : NULL)
{
	for(long i=0; i<n; i++) v[i] = a;
}
...
template <class T>
NRvector<T> & NRvector<T>::operator=(const T val)
// postcondition: normal assignment via copying has been performed;
//		if vector and rhs were different sizes, vector
//		has been resized to match the size of rhs
{
	for (int i=0; i<nn; i++) v[i]=val;
	return *this;
}
...
template <class T>
inline void NRvector<T>::resize(int newn)
{
	if (newn != nn) {
		if (v != NULL) delete[] (v);
		nn = newn;
...
template <class T>
inline void NRvector<T>::resize(long newn)
{
	if (newn != nn) {
		if (v != NULL) delete[] (v);
		nn = newn;
...
template <class T>
inline void NRvector<T>::assign(int newn, const T& a)
{
	if (newn != nn) {
		if (v != NULL) delete[] (v);
		nn = newn;
...
public:
	...
	NRmatrix(long n, long m);			// Zero-based array
	NRmatrix(long n, long m, const T &a);	//Initialize to constant
	NRmatrix(long n, long m, const T *a);	// Initialize to array

	NRmatrix(const NRmatrix &rhs);		// Copy constructor
	NRmatrix & operator=(const NRmatrix &rhs);	//assignment
	NRmatrix & operator=(const T val);	//constant assignment
	typedef T value_type; // make T available externally
	...
	void resize(long newn, long newm); //codigo propio
...
template <class T>
NRmatrix<T>::NRmatrix(long n, long m) : nn(n), mm(m), v(n>0 ? new T*[n] : NULL)
{
	long i,nel=m*n;
	if (v) v[0] = nel>0 ? new T[nel] : NULL;
	for (i=1;i<n;i++) v[i] = v[i-1] + m;
...
template <class T>
NRmatrix<T>::NRmatrix(long n, long m, const T &a) : nn(n), mm(m), v(n>0 ? new T*[n] : NULL)
{
	long i,j,nel=m*n;
	if (v) v[0] = nel>0 ? new T[nel] : NULL;
	for (i=1; i< n; i++) v[i] = v[i-1] + m;
...
template <class T>
NRmatrix<T> & NRmatrix<T>::operator=(const T val)
// postcondition: normal assignment via copying has been performed;
//		if matrix and rhs were different sizes, matrix
//		has been resized to match the size of rhs
{
	int i,j;
	for (i=0; i< nn; i++) for (j=0; j<mm; j++) v[i][j] = val;
	return *this;
}
...
template <class T>
void NRmatrix<T>::resize(long newn, long newm)
{
	long i,nel;
	if (newn != nn || newm != mm) {
		if (v != NULL) {
...
public:
	...
	inline void resize(int newn,int newm, int newk);
	inline void assign(int newn, int newm, int newk, const T& a);
	inline T** operator[](const int i);	//subscripting: pointer to row i
...
void NRMat3d<T>::resize(int newn, int newm, int newk)
{
	...
	if (newn != nn || newm != mm || newk != kk) {
		...
		v = nn>0 ? new T**[nn] : NULL;
		nel = mm*nn*kk;
		if (v) v[0] = nel>0 ? new T*[mm*nn] : NULL;
		if (v) v[0][0] = nel>0 ? new T[mm*nn*kk] : NULL;
...
//asign
template <class T>
void NRMat3d<T>::assign(int newn, int newm, int newk, const T& a)
{
	int i,j,k,nel;
	if (newn != nn || newm != mm || newk != kk) {
		if (v != NULL) {
...
// funcion propia, inicializacion ceros para arreglo de conductividad
template <class T>
void NRMat3d<T>::zeros()
{
	int i, j, k, m;
...
public:
	NRMat4d();
	NRMat4d(int n, int m, int k, int l);
	inline void resize(int newn,int newm, int newk, int newl);
	inline void assign(int newn,int newm, int newk, int newl, const T& a);
	inline void zeros();
	inline T*** operator[](const int i);	//subscripting: pointer to row i
	inline const T* const * operator[](const int i) const;
	inline int dim1() const;
...
template <class T>
NRMat4d<T>::NRMat4d(): nn(0), mm(0), kk(0), ll(0), v(NULL) {}

template <class T>
NRMat4d<T>::NRMat4d(int n, int m, int k, int l) : nn(n), mm(m), kk(k), ll(l), v(new T***[n])
{
	int i,j,p,q;
	v[0] = new T**[n*m];
	v[0][0] = new T*[n*m*k];
	v[0][0][0] = new T[n*m*k*l];
	for(p=1; p<k; p++) v[0][0][p] = v[0][0][p-1] + l;
	for(j=1; j<m; j++) {
		v[0][j] = v[0][j-1] + k;
		v[0][j][0]=v[0][j-1][0]+k*l;
		for(q=1; q<k; q++) v[0][j][q]=v[0][j][q-1]+l;
	}
	for(i=1; i<n; i++) {
		v[i] = v[i-1] + m;
		v[i][0] = v[i-1][0] + m*k;
		...
		for(j=1; j<m; j++) {
			...
			for(p=1; p<k; p++) v[i][j][p]=v[i][j][p-1]+l;
		}
...
template <class T>
void NRMat4d<T>::resize(int newn, int newm, int newk, int newl)
{
	int i,j,p,nel;
	if (newn != nn || newm != mm || newk != kk || newl != ll) {
		if (v != NULL) {
			delete[] (v[0][0][0]);
			delete[] (v[0][0]);
			delete[] (v[0]);
		...
		nel = mm*nn*kk*ll;
		if (v) v[0] = nel>0 ? new T**[mm*nn] : NULL;
		if (v[0]) v[0][0] = nel>0 ? new T*[mm*nn*kk] : NULL;
		if (v[0][0]) v[0][0][0] = nel>0 ? new T[mm*nn*kk*ll] : NULL;
		for(p=1; p<kk; p++) v[0][0][p] = v[0][0][p-1] + ll;
		for(j=1; j<mm; j++) {
			v[0][j] = v[0][j-1] + kk;
			v[0][j][0]=v[0][j-1][0]+kk*ll;
			for(p=1; p<kk; p++) v[0][j][p]=v[0][j][p-1]+ll;
		}
		for(i=1; i<nn; i++) {
			v[i] = v[i-1] + mm;
			v[i][0] = v[i-1][0] + mm*kk;
			v[i][0][0] = v[i-1][0][0] + mm*kk*ll;
			for (p=1; p<kk; p++) {
				v[i][0][p]=v[i][0][p-1]+ll;
			}
			for(j=1; j<mm; j++) {
				v[i][j] = v[i][j-1] + kk;
				v[i][j][0] = v[i][j-1][0] + kk*ll;
				for(p=1; p<kk; p++) v[i][j][p]=v[i][j][p-1]+ll;
			}
...
template <class T>
void NRMat4d<T>::assign(int newn, int newm, int newk, int newl, const T& a)
{
	int i,j,k,m,p,nel;
	if (newn != nn || newm != mm || newk != kk || newl != ll) {
		if (v != NULL) {
...
template <class T>
inline const T* const * NRMat4d<T>::operator[](const int i) const
{
	return v[i];
}
...
typedef const NRMat4d<Doub> Mat4DDoub_I;
typedef NRMat4d<Doub> Mat4DDoub;
