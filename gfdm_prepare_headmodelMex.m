...
function vol = gfdm_prepare_headmodelMex(cfg)
% gfdm_prepare_headmodel.m
% Ernesto Cuartas M (ECM), 06/04/2017
% Updated: 02/06/2020
% Email:  ecuartasm@gmail.com
...
% Casting BioCalc as a Matlab function - precompiled mex function.
if ispc
    [A_m, IA_m, JA_m, cidx_m] = BioCalc([cfg.folder filesep 'tmp'], [cfg.folder filesep 'tmp']);
elseif ismac    
    [A_m, IA_m, JA_m, cidx_m] = BioCalcUnix([cfg.folder filesep 'tmp'], [cfg.folder filesep 'tmp']);
elseif isunix
    [A_m, IA_m, JA_m, cidx_m] = BioCalcUnix([cfg.folder filesep 'tmp'], [cfg.folder filesep 'tmp']);
end
...
cfg_M = [];
cfg_M.Size = size(segment);
cfg_M.A    = double(A_m);
cfg_M.IA   = double(IA_m);
cfg_M.JA   = double(JA_m);
cfg_M.cidx = double(cidx_m);

[ mA, c_idx, Cidx, Nz] = SetStiffnessMatrixMex( cfg_M );
